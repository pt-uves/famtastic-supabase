export interface InviteExistingMemberParams {
  inviter_name: string;
  child_names: string;
  role_label: string;
}
...
<head>
  ...
  <title>New child access on Famtastic</title>
...
<body style="margin:0;padding:0;background-color:#F5F3FF;">
  <div style="display:none;max-height:0;overflow:hidden;opacity:0;">{{ inviter_name }} linked you to {{ child_names }} on Famtastic. Just log in to see their profiles.</div>
        ...
        <table role="presentation" width="480" cellpadding="0" cellspacing="0" border="0" style="width:480px;max-width:480px;background-color:#ffffff;border-radius:20px;border:1px solid #ECE8FA;overflow:hidden;">
          ...
          <tr>
            <td style="padding:24px 40px 8px;text-align:center;">
              <h1 style="margin:0 0 12px;font-family:'Plus Jakarta Sans',-apple-system,Segoe UI,Helvetica,Arial,sans-serif;font-size:24px;line-height:1.25;font-weight:700;color:#2B2340;">You've got new child access</h1>
              <p style="margin:0 0 14px;font-family:'Plus Jakarta Sans',-apple-system,Segoe UI,Helvetica,Arial,sans-serif;font-size:15px;line-height:1.6;color:#6B6480;"><strong style="color:#2B2340;">{{ inviter_name }}</strong> has linked you to <strong style="color:#2B2340;">{{ child_names }}</strong> as a <strong style="color:#6438E8;">{{ role_label }}</strong> on Famtastic.</p>
              <p style="margin:0;font-family:'Plus Jakarta Sans',-apple-system,Segoe UI,Helvetica,Arial,sans-serif;font-size:15px;line-height:1.6;color:#6B6480;">You already have a Famtastic account, so just open the app and log in - their profiles will be waiting for you.</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:28px 40px 34px;text-align:center;">
              <div style="border-top:1px solid #ECE8FA;padding-top:22px;">
                ...
                <p style="margin:0;font-family:'Plus Jakarta Sans',-apple-system,Segoe UI,Helvetica,Arial,sans-serif;font-size:12px;line-height:1.5;color:#A29DB4;">This is an automated notification from Famtastic.</p>
...
export function getInviteExistingMemberHtml(
  params: InviteExistingMemberParams,
): string {
